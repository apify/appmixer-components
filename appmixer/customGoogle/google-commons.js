...
function transformResponse(value) {
    ...
    parser.parseString(value, (err, result) => {
        ...
        // backwards compatibility of result after migration
        // from: https://www.npmjs.com/package/feedme
        // to: https://www.npmjs.com/package/xml2js
        if (result.entry && !result.items) {
            result.items = result.entry;
            delete result.entry;
        }
...
function getGmailPartContent(part, _content) {
    ...
    // _content obejct is used to collect returned values during the recursive walk.
    _content = _content || {
        text: '',
        html: ''
    };
